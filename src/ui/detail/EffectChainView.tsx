...
import { Plus, Trash2, X, Search, Music, ChevronUp, ChevronDown, Power, PowerOff } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
import type { BusInfo, PluginInstance } from './types';
...
function SortablePluginRow({
  ...
  return (
    ...
    >
      ...
      >
        {plugin.enabled ? <Power className="w-2.5 h-2.5" /> : <PowerOff className="w-2.5 h-2.5" />}
      </button>
      <button
        onClick={(e) => {
          e.stopPropagation();
        ...
        className={`w-4 h-4 rounded flex items-center justify-center text-slate-500 transition-colors ${
          !canMoveUp ? 'opacity-30 cursor-not-allowed' : 'hover:text-slate-300 hover:bg-slate-700/40'
        }`}
      ...
      <button
        ...
        className={`w-4 h-4 rounded flex items-center justify-center text-slate-500 transition-colors ${
          !canMoveDown ? 'opacity-30 cursor-not-allowed' : 'hover:text-slate-300 hover:bg-slate-700/40'
        }`}
...
export default function EffectChainView({
  bus,
  onPluginsChange,
}: {
  bus: BusInfo;
  ...
  const handleOpenPluginBrowser = async () => {
    setLoading(true);
    try {
      const list = await getAvailablePlugins();
  ...
  const handleSetPluginEnabled = async (instanceId: string, enabled: boolean) => {
    ...
    setLocalPlugins((current) => current.map((p) => (p.id === instanceId ? { ...p, enabled } : p)));
    ...
    <>
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-3 py-2 border-b border-slate-700/50">
          <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Effect Chain</div>
        </div>

        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1">
          {localPlugins.length === 0 ? (
            <div className="text-[10px] text-slate-600 text-center py-4">No effects loaded</div>
          ) : (
            <DndContext
              sensors={sensors}
            ...
            >
              <SortableContext items={localPlugins.map((p) => p.id)} strategy={verticalListSortingStrategy}>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="w-[480px] max-h-[600px] bg-slate-900 rounded-xl border border-slate-700 shadow-2xl flex flex-col">
                ...
                <div className="space-y-4">
                  {manufacturers.map((manufacturer) => (
                    <div key={manufacturer}>
                      <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-2 py-1">{manufacturer}</div>
                      <div className="space-y-1">
                        {byManufacturer[manufacturer].map((plugin) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-sm font-medium text-white truncate group-hover:text-purple-300">{plugin.name}</div>
            ...
            <div className="p-3 border-t border-slate-700 bg-slate-900/50">
              <div className="text-[10px] text-slate-500 text-center">{availablePlugins.length} plugins available</div>
            </div>
          </div>
        </div>
      ) : null}
