...
**問題点（旧設計）:**
- UI コンポーネント（SpectrumLayout）がオーディオエンジンの初期化を担当
- React ライフサイクル（useEffect）に依存
- Strict Mode の2重実行の影響を受ける
- 関心の分離が不十分
...
│              Tauri Backend (.setup())                   │
│                                                         │
│  1. App起動時に自動的にオーディオエンジン初期化          │
│  2. デフォルトデバイス（deviceId: 0）で音声開始          │
│  3. 状態をフロントエンドに公開                           │
│                                                         │
└─────────────────────────────────────────────────────────┘
                          ↓ 状態取得
...
│                   Frontend (React)                      │
│                                                         │
│  - バックエンドの状態を表示（get_output_runtime）        │
│  - ユーザー操作でデバイス切り替え（RightPanel）          │
│  - 初期化は行わない（責務外）                            │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|_app| {
            ...
            // Start audio output with default device (deviceId 0 = auto-select)
            match crate::v2::audio::output::start_output(0) {
                Ok(_) => {
                    println!("[Spectrum] Audio engine initialized successfully");
                }
                Err(e) => {
                    eprintln!("[Spectrum] Warning: Failed to initialize audio engine: {}", e);
                    // Don't fail app startup if audio init fails
                }
            }
...
#### Frontend: `src/ui/RightPanel.tsx`

```typescript
// ユーザーがデバイスを切り替えた時のみ stop → start
useEffect(() => {
...
| 項目 | 旧設計（UI起点） | 新設計（Backend起点） |
|------|------------------|----------------------|
| **初期化タイミング** | UIマウント時 | アプリ起動時 |
| **Reactライフサイクル依存** | あり | なし |
| **Strict Mode影響** | 2重実行 | 影響なし |
| **関心の分離** | 混在 | 明確 |
| **初期化失敗時の挙動** | UIが表示されない | アプリは起動、エラーログ表示 |

### デバイス切り替えフロー

```
1. ユーザーがRightPanelでデバイス選択
   ↓
2. useEffect が prev !== '' を検出
   ↓
3. stopOutput(prev) → startOutput(current)
   ↓
4. バックエンドでデバイス切り替え
   ↓
5. get_output_runtime() で状態取得
   ↓
6. UIに反映
```

初回マウント時（prev === ''）は何もしない → バックエンドで既に起動済みのため
